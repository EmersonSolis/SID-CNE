...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/cne/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.cne.com docker_peer0.oea.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/cne/msp organizations/fabric-ca/cne/tls-cert.pem organizations/fabric-ca/cne/ca-cert.pem organizations/fabric-ca/cne/IssuerPublicKey organizations/fabric-ca/cne/IssuerRevocationPublicKey organizations/fabric-ca/cne/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/oea/msp organizations/fabric-ca/oea/tls-cert.pem organizations/fabric-ca/oea/ca-cert.pem organizations/fabric-ca/oea/IssuerPublicKey organizations/fabric-ca/oea/IssuerRevocationPublicKey organizations/fabric-ca/oea/fabric-ca-server.db'
